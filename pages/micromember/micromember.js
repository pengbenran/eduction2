    ...
    if (e.currentTarget.dataset.url =="../membershioup/membershioup"){
      wx.request({
        ...
        success: function (res) {
          if (res.data.code == 0) {
            var flag=true; 
            console.log(res.data.memberLvList[0].discount * 1)
            console.log(res.data.memberLvList[1].discount * 1)
            console.log(res.data.memberLvList[0].discount*1 > that.data.distribeDo.discount*1)
            console.log(res.data.memberLvList[1].discount*1 > that.data.distribeDo.discount*1)
            if (res.data.memberLvList[0].discount > that.data.distribeDo.discount || res.data.memberLvList[1].discount > that.data.distribeDo.discount) {
              flag=true
            }
            else{
              flag=false
