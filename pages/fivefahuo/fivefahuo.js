...
Page({
  data: {
    ...
    tabitem: [{ name: '待付款', isSelect: true, status: 0 }, { name: '已付款', isSelect: false, status: 1}, { name: '已取消', isSelect: false, status: 6 }, { name: '退款中', isSelect: false, status: 9 }, { name: '已退款', isSelect: false, status: 12 }, { name: ' 已完成', isSelect: false, status: 5 }]
  },
  // 刷新
  onPullDownRefresh() {
  ...
  changTab:function(e){
    ...
    that.orderRequest(e.currentTarget.dataset.status)
  },
  orderRequest: function (status){
    var that=this 
    var orderstatus={};
    orderstatus.memberId=that.data.memberId;
    orderstatus.status=status
    wx.request({
      url: api + '/api/order/apiSelectOrderList',
      data: {
        parms: JSON.stringify(orderstatus)
      },
      headers: {
